...
class DACCAS:
    def __init__(
        ...
        # Optionally load every model now so the first Solve() is instant.
        if preload:
            self.warmup()

    # ----------------------------------------------------------------------- #
    # Eager loading: load the classifier and every solver up front.
    # Call once at startup (or pass preload=True) so later Solve() calls don't
    # pay first-use weight-loading / ConvNeXt-download cost.
    # ----------------------------------------------------------------------- #
    def warmup(self, verbose: bool = False) -> "DACCAS":
        if verbose:
            print("[DACCAS] loading classifier ...")
        self.classifier.load()
        for name, solver in self.registry.items():
            if verbose:
                print(f"[DACCAS] loading solver: {name} ...")
            solver.load()
        if verbose:
            print("[DACCAS] warmup complete.")
        return self
